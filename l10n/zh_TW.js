OC.L10N.register(
    ...
    "Upload file to Nextcloud (choose destination)" : "上傳檔案至 Nextcloud（選擇目的地）",
    ...
    "Select upload destination" : "選取上傳目的地",
    "User not authenticated" : "使用者未經認證",
    "Threads" : "討論串",
    "Replies" : "回覆",
    ...
    "Pick a folder …" : "挑選資料夾……",
    "Files attached to threads or replies will be uploaded to this directory in your Nextcloud files. The path resolves to your own files; other users will see the same folder at its location in their own files." : "附加於討論串或回覆中的檔案將上傳至您 Nextcloud 檔案中的此目錄。此路徑指向您自己的檔案；其他使用者在他們自己的檔案中，也會看到位於相同位置的相同資料夾。",
    "Upload behavior" : "上傳行為",
    "Always upload to chosen directory" : "一律上傳至選定的目錄",
    "Always prompt for upload location" : "一律提示選擇上傳位置",
    "Whether attachment uploads use your configured directory or ask you to pick a destination each time" : "附件上傳時，系統會使用您設定的目錄，或是每次都會要求您選擇儲存位置",
    "Use category-specific paths when available" : "若可用，請使用分類專屬的路徑",
    "When a category has its own attachments folder configured, uploads in that category go there instead of your default upload directory" : "當某個分類已設定專屬的附件資料夾時，該分類中的上傳檔案會存放在該資料夾中，而非預設的上傳目錄",
    "Browse" : "瀏覽",
    ...
    "Attachments" : "附件",
    "Configure where uploads in this category are stored" : "設定此分類中上傳檔案的儲存位置",
    "Use default attachments path" : "使用預設附件路徑",
    "Uploads in this category land in each user's personal upload directory" : "此分類中的上傳檔案會存放在每位使用者的個人上傳目錄中",
    "Use category-specific attachment path" : "使用分類專屬的附件路徑",
    "Category attachments folder" : "分類附件資料夾",
    "The shown path is resolved to your own files. Other admins will see the same folder at the location it appears in their own files." : "顯示的路徑會根據您自身的檔案進行解析。其他管理員所看到的同一個資料夾，將顯示在其自身檔案中的對應位置。",
    "This folder must be accessible to every user who uploads in this category. If a user cannot write to it, their upload will fall back to their personal default path." : "所有在此分類中上傳檔案的使用者都必須能夠存取此資料夾。若使用者無法寫入此資料夾，其上傳的檔案將自動轉存至其個人預設路徑。",
    "Pick a folder" : "挑選資料夾",
    ...
    "Category counts" : "分類計數",
    "Configure how thread and reply counts are calculated for categories" : "設定分類中的討論串與回覆數量計算方式",
    "Count subcategory threads/replies in category counts" : "在分類計數中計算子分類的討論串/回覆數量",
    "When enabled, category cards will show total threads/replies from all subcategories combined. When disabled, only the threads/replies from the category itself will be counted." : "啟用後，分類卡片會顯示所有子分類合計的討論串/回覆總數。若停用，僅會計算該分類本身的討論串/回覆數量。",
    ...
    "Newest first" : "新的在前",
    "Oldest first" : "舊的在前",
    "Permanently delete this thread and all its replies? This cannot be undone." : "要永久刪除此討論串及其所有回覆嗎？此操作無法復原。",
    "Permanently delete this reply? This cannot be undone." : "永久刪除此回覆？此操作無法復原。",
    "Failed to permanently delete thread" : "永久刪除討論串失敗",
    "Failed to permanently delete reply" : "永久刪除回覆失敗",
    "Failed to restore thread" : "還原討論串失敗",
    "Failed to restore reply" : "還原回覆失敗",
    "_Permanently delete %n selected thread and all its replies? This cannot be undone._::_Permanently delete %n selected threads and all their replies? This cannot be undone._" : ["要永久刪除 %n 個選定的討論串與其所有的回覆嗎？此操作無法撤銷。"],
    "_Permanently delete %n selected reply? This cannot be undone._::_Permanently delete %n selected replies? This cannot be undone._" : ["要永久刪除 %n 個選定的回覆嗎？此操作無法復原。"],
    "_Deleted %n item; the rest could not be removed._::_Deleted %n items; the rest could not be removed._" : ["已刪除 %n 個項目：剩下的無法刪除。"],
    "_Permanently deleted %n item._::_Permanently deleted %n items._" : ["已永久刪除 %n 個項目。"],
    "Failed to permanently delete the selected items" : "永久刪除選定的項目失敗",
    "Create role" : "建立角色",
    "Edit role" : "編輯角色",
